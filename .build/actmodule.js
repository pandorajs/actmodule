define("pandora/actmodule/1.0.0/actmodule", [ "$", "./core", "pandora/widget/1.0.0/widget", "pandora/base/1.0.0/base", "pandora/class/1.0.0/class", "pandora/events/1.0.0/events", "gallery/handlebars/1.3.0/handlebars-runtime", "./utils", "./jquery.zclip", "./template/pc/style1.handlebars", "./template/pc/style2.handlebars", "./template/pc/style3.handlebars", "./template/pc/style4.handlebars", "./template/pc/comment.handlebars", "./template/pc/imageCaptcha.handlebars", "./template/pc/info.handlebars", "./template/pc/result.handlebars", "./template/pc/sms.handlebars", "./template/pc/vote.handlebars", "./template/m/style1.handlebars", "./template/m/style2.handlebars", "./template/m/style3.handlebars", "./template/m/comment.handlebars", "./template/m/imageCaptcha.handlebars", "./template/m/info.handlebars", "./template/m/result.handlebars", "./template/m/sms.handlebars", "./template/m/vote.handlebars" ], function(require, exports, module) {
    /**
	* 通用活动组件 
	* @module ActModule
	*/
    var $ = window.$ = window.jQuery = require("$"), Core = require("./core"), Utils = require("./utils"), ZeroClipboard = window.ZeroClipboard = require("./jquery.zclip");
    /**
	* 表现层类
	*
	* @class ActModule
    ...
    var ActModule = Core.extend({
        ...
        closePopup: function(e) {
            if (this.mobile) {
                ...
                var $popup = $(e.currentTarget).parent(), $mask = $popup.next(".zq-actcompt-pop-mask");
                $popup.remove();
                $mask.remove();
            }
        },
        ...
        checkStatus: function(beginTime, endTime) {
            var beginTimeUnix = Utils.datetimeToUnix(beginTime), endTimeUnix = Utils.datetimeToUnix(endTime), nowUnix = ~~($.now() / 1e3), winnerUrl = this.option("winnerUrl") ? this.option("winnerUrl") : false;
        ...
        render: function(actInfo) {
            var self = this, startTime = this.option("collectInfo") ? actInfo.beginTime : actInfo.lotteryBeginTime, endTime = this.option("collectInfo") ? actInfo.endTime : actInfo.lotteryEndTime;
        ...
        submitVote: function(submit) {
            ...
            var voteid = $vote.data("voteid"), voteitem = [];
        ...
        showCommentPop: function() {
            var self = this, data = {
                commentTip: self.option("commentTip")
            };
        ...
        submitComment: function() {
            var val = $.trim($("#comment_text").val()), sid = $("#SOHUCS").attr("sid");
            if (!sid) {
                return;
            }
            if (val === "") {
        ...
        validateMobile: function() {
            var reMobile = /^1\d{10}$/, mobile = $("#mobile_input").val();
        ...
        validateInfoForm: function() {
            var pass = true, formData = {}, reMail = /^[\w-]+(\.[\w-]+)*@[\w-]+(\.[\w-]+)+$/, reMobile = /^1\d{10}$/, reQQ = /^[1-9]\d{4,11}$/, $input = this.mobile ? $(".zq-actcompt-form-input") : $(".zq-actcompt-form-input input");
            $input.each(function() {
                var val = $.trim($(this).val()), name = $(this).attr("name"), cName = $(this).parents(".zq-actcompt-form-item").find("[data-name]").text(), $error = this.mobile ? $(this).parent().next(".zq-actcompt-form-error") : $(this).parents(".zq-actcompt-form-item").next(".zq-actcompt-form-error"), $errorText = this.mobile ? $error : $error.find("span");
                if (val === "") {
                    $error.show();
        ...
        submitImageCaptcha: function() {
            var self = this, captcha = $.trim($("#captcha_input").val());
            if (captcha.length < 4) {
                alert("请输入正确的验证码");
                return;
            }
        ...
        animateEnd: function(data) {
            ...
            if (!this.option("needInfo") && !data.code) {
                //实物
