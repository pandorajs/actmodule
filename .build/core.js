define("pandora/actmodule/1.0.0/core", [ "$", "pandora/widget/1.0.0/widget", "pandora/base/1.0.0/base", "pandora/class/1.0.0/class", "pandora/events/1.0.0/events", "gallery/handlebars/1.3.0/handlebars-runtime", "./utils" ], function(require, exports, module) {
    var $ = require("$"), Widget = require("pandora/widget/1.0.0/widget"), Handlebars = require("gallery/handlebars/1.3.0/handlebars-runtime"), Utils = require("./utils");
    /**
	* 数据及基础类
	*
	* @class Core
	...
	*/
    var Core = Widget.extend({
        ...
        setup: function() {
            var host = this.host = "http://p.act.dev.17173.com/api/v1/activity/" + this.option("actId"), lotteryId = this.option("lotteryId"), fieldSetId = this.option("fieldSetId");
            this.urls = {
		 ...
		 */
        isMobile: function() {
            var ua = navigator.userAgent, os = [ "Android", "iPhone", "SymbianOS", "Windows Phone", "iPod", "BlackBerry" ], mobile = false;
        ...
        getInfoFields: function() {
            ...
            $.getJSON(self.urls.fieldset, {
                formId: this.option("fieldSetId")
            }, function(data) {
        ...
        doComment: function(val, sid) {
            ...
            $.getJSON(this.urls.cyanLoad, {
                ...
                $.getJSON(self.urls.commentSubmit, {
                    ...
                    access_token: "P9YIlfiDcC45SbJInp7DCfvA-Bhgu1ZG"
                }, function(json) {
                    if (!self.option("collectInfo")) {
                        //如果是即时抽奖
        ...
        submitInfo: function(commentData) {
            if (Math.floor(new Date().getTime() / 1e3) > this.endTime) {
                alert("活动已结束.");
                return;
            }
            var self = this, formData = commentData || this.validateInfoForm();
            if (!!formData) {
                var info = {
        ...
        handleLotteryResult: function(data) {
            ...
            switch (data.result) {
              ...
              case "info.lottery.success":
                //中奖
                data.success = true;
                resultType = 1;
                break;

              case "info.login.error":
                //未登录
                resultType = 2;
                break;

              case "info.captcha.error":
                //验证码错误
                resultType = 2;
                break;

              case "info.mobile.error":
                //手机号错误
                resultType = 2;
                break;
